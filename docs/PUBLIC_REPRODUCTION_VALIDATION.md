...
The fixed-input files are intentionally excluded from the software repository. The STATS19 source-terms review is complete; the CAS review, immutable public archive URLs and data-record DOI remain pending. Until those gates are complete, a new user must obtain the exact files separately and pass the recorded hashes; the software release is not yet a self-contained one-command download-and-run package.
