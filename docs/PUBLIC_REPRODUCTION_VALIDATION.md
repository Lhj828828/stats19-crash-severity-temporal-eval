...
The fixed-input files are intentionally excluded from the software repository. Their immutable public archive URLs and data-record DOI remain pending source-terms review and archive publication. Until that gate is complete, a new user must obtain the exact files separately and pass the recorded hashes; the software release is not yet a self-contained one-command download-and-run package.
